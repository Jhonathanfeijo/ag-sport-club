...
const ModalAddTipoQuadra = ({
  tiposQuadra,
  setTiposQuadra,
  setIsModalAddTipoQuadraOpen,
  setRender,
}) => {
  ...
  const postTipoQuadra = data => {
    ...
    let toastId = toast.loading('Adicionando tipo de quadra', {
      isLoading: true,
    });
    const postData = async () => {
      await api
        .post('tipo_quadra', data, { headers })
        .then(json => {
          console.log(json);
    ...
    <>
      <div className='fixed bg-third/15 top-0 left-0 w-screen h-screen flex flex-col justify-center md:justify-start items-center'>
        <div className='bg-secundary md:mt-40 px-5 rounded shadow-xl drop-shadow-xl'>
          ...
          <form
            onSubmit={handleSubmit(postTipoQuadra)}
            className='flex flex-col my-3  lg:w-[500px]'
            action=''
          >
            ...
            <div className='flex flex-row mt-2 gap-2 self-end'>
              <button
                type='button'
                onClick={() => setIsModalAddTipoQuadraOpen(false)}
                className='px-2 py-1 rounded bg-third/10 font-medium text-[#000000]'
              >
                Cancelar
              </button>
              <button
