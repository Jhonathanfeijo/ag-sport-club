...
const ModalEditTipoQuadra = ({
  tipoQuadraToEdit,
  setTipoQuadraToEdit,
  setIsModalEditTipoQuadraOpen,
  tiposQuadra,
  setTiposQuadra,
  setRender,
}) => {
  ...
  const editTipoQuadra = data => {
    ...
    const putData = async () => {
      ...
      await api
        .put(`tipo_quadra/${tipoQuadraToEdit.idTipoQuadra}`, data, { headers })
        .then(json => {
          console.log(json);
          ...
          tiposQuadraAux = tiposQuadraAux.filter(
            tipoQuadraAux =>
              tipoQuadraAux.idTipoQuadra != tipoQuadraToEdit.idTipoQuadra,
          );
    ...
    <>
      <div className='fixed bg-third/15 top-0 left-0 w-screen h-screen flex flex-col justify-center md:justify-start items-center'>
        <div className='bg-secundary md:mt-40 px-5 rounded shadow-xl drop-shadow-xl max-w-full w-[450px]'>
          ...
          <form
            onSubmit={handleSubmit(editTipoQuadra)}
          ...
          >
            ...
            <div className='flex flex-row gap-2 my-3 self-end'>
              <button
                type='button'
                onClick={() => setIsModalEditTipoQuadraOpen(false)}
                className='flex-1 py-1 px-2 rounded border font-medium'
              >
                Cancelar
              </button>
              <button className='flex-1 py-1 px-3 rounded bg-primary text-secundary font-medium'>
                Editar
              </button>
            </div>
          </form>
