...
const ModalResetPasswordUser = ({ setIsModalRessetPasswordUserOpen }) => {
  ...
  const resetPassword = (data) => {
    ...
    if (data.senhaNova != data.senhaNovaRepetida) {
      toast.error('Senhas não conferem', { style: { fontWeight: 'bold' }, isLoading: false, autoClose: 2500 })
    }
    const dataReset = {
    ...
    const postReset = async () => {
      const toastId = toast.loading("Resetando senha", { style: { fontWeight: 'bold' } })
      await api.post("usuario/resetPassword", dataReset, headers)
        .then(() => {
        ...
        .catch((err) => {
          console.log(err)
          toast.update(toastId, { type: 'error', render: err.response.data.message, autoClose: 2500, isLoading: false, style: { fontWeight: 'bold' } })
        })
    }
    postReset();
  }
    ...
    <>
      <div className='h-screen w-screen fixed top-0 left-0 bg-third/15 z-50 flex justify-center items-center'>
        <div className='w-[350px] bg-secundary rounded drop-shadow-lg shadow-lg max-w-[95%] px-4'>
          ...
          <div>
            <form onSubmit={handleSubmit(resetPassword)} action='font-medium'>
              <div className='flex flex-col'>
                <label className='font-bold' htmlFor=''>
                  Digite a senha atual
                </label>
                <input {...register('senhaAtual')} name="senhaAtual" id="senhaAtual" className='p-1 border rounded' type='password' />
              </div>
              <div className='flex flex-col'>
                <label className='font-bold' htmlFor=''>
                  Digite a senha nova
                </label>
                <input {...register('senhaNova')} id="senhaNova" name="senhaNova" className='p-1 border rounded' type='password' />
              </div>
              <div className='flex flex-col'>
                <label className='font-bold' htmlFor=''>
                  Repita a senha nova
                </label>
                <input {...register('senhaNovaRepetida')} id="senhaNovaRepetida" name="senhaNovaRepetida" className='p-1 border rounded' type='password' />
              </div>
