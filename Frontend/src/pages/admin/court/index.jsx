            ...
            <div className="w-full">
                {isDataLoaded && (
                    <>
                                    ...
                                    <table className="w-full mt-3 mb-2 shadow-lg drop-shadow-lg">
                                        <thead className="sticky">
                                            <tr className="bg-primary sticky text-secundary text-left">
                                                <th className="px-2 sticky py-2 rounded-bl">Descrição</th>
                                        ...
                                        <tbody>
                                            {courtList.map((court, index) => (
                                                <tr className={`${index % 2 == 1 ? "bg-primary/10" : ""} font-bold`} key={index}>
                                                    ...
                                                    <td className="flex flex-row items-center py-1 justify-end text-secundary px-2 gap-2 font-normal">
                                                        <button onClick={() => { setCourtToEdit({...court}); setIsModalEditCourtOpen(true); }} className="bg-primary rounded py-1 px-2">Editar</button>
                                                        <button onClick={() => { setIdCourtDelete(court.idQuadra); setIsModalDeleteCourtOpen(true); }} className="bg-danger/70 rounded py-1 px-1">Deletar</button>
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                        ...
                        <button onClick={() => setIsModalAddCourtOpen(true)} className="w-full py-1.5 text-xl text-secundary bg-primary rounded">Adicionar quadra</button>
                    </>
                )}
            </div>

            {isModalAddCourtOpen && (
                <ModalAddCourt setIsModalAddCourtOpen={setIsModalAddCourtOpen} setCourtList={setCourtList} courtList={courtList} />
            )}

            {isModalDeleteCourtOpen && (
                <ModalDeleteCourt idCourtDelete={idCourtDelete} setIsModalDeleteCourtOpen={setIsModalDeleteCourtOpen} setCourtList={setCourtList} courtList={courtList} />
            )}

            {isModalEditCourtOpen && (
                <ModalEditCourt setIsModalEditCourtOpen={setIsModalEditCourtOpen} setCourtList={setCourtList} courtList={courtList} courtToEdit={courtToEdit} setCourtToEdit={setCourtToEdit} />
            )}
        </>
    );
};
