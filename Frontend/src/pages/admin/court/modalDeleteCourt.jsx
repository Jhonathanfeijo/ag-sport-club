...
const ModalDeleteCourt = ({ setIsModalDeleteCourtOpen, courtToDelete, courtList, setCourtList, setRender }) => {
    ...
    const deleteCourt = () => {
        ...
        const fetchData = async () => {
            await api.delete(`quadra/${courtToDelete.idQuadra}`, headers)
                .then((json) => {
                    console.log(json);
                    setIsModalDeleteCourtOpen(false);
                    let courtListAux = [...courtList];
                    courtListAux = courtListAux.filter((court) => court.idQuadra != courtToDelete.idQUadra);
                    setCourtList(courtListAux);
                    toast.update(toastId, {
                        type: "success",
                        autoClose: 2500,
                        render: "A quadra foi excluida com sucesso",
                        isLoading: false
                    })
        ...
        <>
            <div className="w-screen h-screen fixed top-0 left-0 bg-third/10 flex flex-col justify-center md:justify-start items-center">
                <div className="bg-secundary px-3 w-[400px] md:w-[500px] max-w-[95%] flex flex-col items-start md:mt-40 rounded shadow-lg drop-shadow-lg">
                    ...
                    <h2 className='font-bold w-full text-center'>{`${courtToDelete.locQuadra.toUpperCase()}`}</h2>
