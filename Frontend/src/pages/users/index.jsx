...
const Users = () => {
  ...
  const schema = yup
    .object({
      ...
      senha_novo_usuario: yup
        .string()
        .min(6, 'No minímo 6 caracteres')
        .required(),
      email_novo_usuario: yup
        .string()
        .email('Digite um email válido')
        .required(),
      nome_novo_usuario: yup
        .string()
        .min(3, 'O nome precisa ter mais de 3 letras')
        .required(),
          ...
          <main className=' pt-20 md:ml-10 w-[85%] max-w-[100%] md:w-full h-full flex-1 flex flex-col items-center lg:items-start justify-start'>
            <section className=' my-5 lg:mt-10 mb-2 w-[92%] lg:w-[600px] flex flex-col lg:items-end justify-start lg:flex-row lg:justify-start gap-2 lg:gap-3'>
              <div className='flex-wrap flex flex-col items-start justify-start'>
                <label className='text-lg ' htmlFor='nomeBusca'>
                  Nome
                </label>
                <input
                  className='p-1.5 border border-primary rounded'
                  type='text'
                  name='nomeBusca'
                  id='nomeBusca'
                />
              ...
              <div className=' flex-wrap flex flex-col items-start justify-start'>
                <label className='text-lg ' htmlFor='cpfBusca'>
                  CPF
                </label>
                <input
                  className='w-[150px] max-w-[90%] p-1.5 border border-primary rounded'
                  type='text'
                  name='cpfBusca'
                  id='cpfBusca'
                />
              ...
              <div>
                <button
                  onClick={handleModal}
                  className='py-1.5 px-2 rounded border border-primary bg-primary text-lg text-secundary'
                >
                  Adicionar usuário
                </button>
              </div>
                      ...
                      {Object.keys(usersFiltered[0]).map((key, index) => {
                        return (
                          <th
                            ...
                            } w-full text-left text-secundary px-2 py-5`}
                            key={index}
                          >
                            {key}
                          </th>
                      ...
                      <tr key={index}>
                        ...
                        <>
                          ...
                          <td className='px-2 py-1.5'>
                            <button className=' shadow-sm drop-shadow bg-primary text-secundary p-1 px-2'>
                              Editar
                            </button>
                          </td>
                          <td className='px-2 py-1.5'>
                            <button className=' shadow-sm drop-shadow bg-danger text-secundary  p-1 px-2'>
                              Excluir
                            </button>
                          </td>
                        </>
              ...
              <div className='fixed h-screen w-screen bg-third bg-opacity-10 top-0 left-0 flex justify-center items-start'>
                <div className='bg-secundary w-[90%] md:w-[500px] h-[87%] md:h-[650px] drop-shadow-xl shadow-xl rounded-lg flex flex-col items-center mt-20'>
                  ...
                  <form
                    ...
                    className='w-[90%] md:w-[75%] flex flex-col justify-start items-start gap-1 text-base font-semibold'
                    action=''
                  >
