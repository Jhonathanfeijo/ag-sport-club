...
const ModalSeeMoreAboutReserv = ({ setRender, reserv, setMyReservs, myReservs, setIsModalSeeMoreAboutReservOpen }) => {
        ...
        <div className="fixed top-0 left-0 h-screen w-screen bg-third/15 flex flex-col items-center justify-center">
            <div className="bg-secundary rounded drop-shadow-xl shadow-lg p-4 sm:w-[400px] lg:w-[900px] font-bold flex flex-col items-center relative">
                {isModalConfirmAlterReservStatusOpen && (
                    <ModalConfirmAlterReservStatus setRender={setRender} setIsModalSeeMoreAboutReservOpen={setIsModalSeeMoreAboutReservOpen} action={action} myReservs={myReservs} reserv={reserv} setIsModalConfirmAlterReservStatusOpen={setIsModalConfirmAlterReservStatusOpen} setMyReservs={setMyReservs} />
                )}
                ...
                <div className="flex flex-row flex-wrap gap-3">
                    ...
                    <div className="flex flex-col gap-1 flex-1">
                        ...
                        <input disabled className="rounded px-1 border py-1" value={`${reserv.horarioInicial < 10? `0${reserv.horarioInicial}`:`${reserv.horarioInicial}`}:00 - ${reserv.horarioInicial + 1 < 10? `0${reserv.horarioInicial+1}`:`${reserv.horarioInicial+1}`}:00`} type="text" />
                    </div>
                    <div className="flex flex-col gap-1 flex-1">
                ...
                <div className="self-end flex flex-row items-end justify-end gap-2 my-4 font-medium w-full">
                    ...
                    {reserv.status.toUpperCase() === 'PENDENTE' ?
                        (<div className="flex flex-row items-end gap-2">
                            <button type="button" onClick={() => { setAction("pay"); setIsModalConfirmAlterReservStatusOpen(true) }} className="py-1 px-3 bg-sucess rounded text-secundary">Pagar reserva</button>
                            <button type="button" onClick={() => { setAction("delete"); setIsModalConfirmAlterReservStatusOpen(true) }} className="py-1 px-2 bg-danger/70 rounded text-secundary">Cancelar reserva</button>
                        </div>) : (<>
                        </>)
                    }
                </div>
