...
const ModalConfirmAlterReservStatus = ({setRender, myReservs, action, setMyReservs, reserv, setIsModalConfirmAlterReservStatusOpen, setIsModalSeeMoreAboutReservOpen}) => {
    ...
    return (<>
        ...
        <div className="w-full h-full absolute flex flex-row justify-center items-center top-0 left-0 bg-third/15">
            <div className="px-10 py-6 bg-secundary rounded drop-shadow-lg shadow-lg flex flex-col">
                ...
                <span className="self-center underline my-4 text-lg">{`Quadra ${reserv.quadraLoc.toUpperCase()} - Data: ${reserv.dataLocacao} às ${reserv.horarioInicial < 10? `0${reserv.horarioInicial}`:`${reserv.horarioInicial}`}:00 - ${reserv.horarioInicial + 1 < 10? `0${reserv.horarioInicial+1}`:`${reserv.horarioInicial+1}`}:00`}</span>
