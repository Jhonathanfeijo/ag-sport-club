...
const ModalConfirmEditMyProfile = ({
        ...
        >
          <div className='bg-secundary rounded p-4 flex flex-col w-[550px] max-w-[95vw] shadow-lg drop-shadow'>
            <h2 className='mb-4 font-bold text-xl'>
              Voce tem certeza que deseja atualizar seus dados ?
            </h2>
            <span className='max-w-full break-words font-bold'>{`Nome: ${capitalizeFirstLetter(
              userInfo.nome,
            )}`}</span>
            ...
            <div className='flex flex-row items-center font-medium self-end gap-2 mt-4'>
              <button
                className='rounded px-2 py-1'
                onClick={() => setIsModalConfirmEditMyProfileOpen(false)}
              >
                Fechar
              </button>
              <button onClick={() => editUser()} className='rounded px-2 py-1'>
                Atualizar
              </button>
            </div>
          </div>
