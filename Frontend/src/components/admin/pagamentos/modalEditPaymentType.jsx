...
const ModalEditPaymentType = ({
  paymentTypeToEdit,
  setPaymentTypeToEdit,
  setIsModalEditPaymentTypeOpen,
  paymentTypes,
  setPaymentTypes,
  setRender,
}) => {
  ...
  const editPaymentType = data => {
    ...
    const putData = async () => {
      ...
      await api
        .put(`form_pagamento/${paymentTypeToEdit.idFormPagamento}`, data, {
          headers,
        })
        .then(json => {
          console.log(json);
          ...
          paymentTypesAux = paymentTypesAux.filter(
            paymentTypeAux =>
              paymentTypeAux.idFormPagamento !=
              paymentTypeToEdit.idFormPagamento,
          );
    ...
    <>
      <div className='fixed bg-third/15 top-0 left-0 w-screen h-screen flex flex-col justify-center md:justify-start items-center'>
        <div className='bg-secundary md:mt-40 px-5 rounded shadow-xl drop-shadow-xl max-w-full w-[450px]'>
          ...
          <form
            onSubmit={handleSubmit(editPaymentType)}
          ...
          >
            ...
            <div className='flex flex-row items-center gap-3 my-3'>
              ...
              <label class='inline-flex items-center cursor-pointer'>
                <input
                  ...
                  value={paymentTypeToEdit.ativo}
                  class='sr-only peer'
                  checked={paymentTypeToEdit.ativo}
                />
            ...
            <div className='flex flex-row gap-2 self-end my-3'>
              <button
                type='button'
                onClick={() => setIsModalEditPaymentTypeOpen(false)}
                className='flex-1 lg:flex-none py-1 px-2 rounded border font-medium'
              >
                Cancelar
              </button>
              <button className='flex-1 lg:flex-none py-1 rounded bg-primary px-4 text-secundary font-medium'>
                Editar
              </button>
            </div>
          </form>
